...
1. **Install NSSM**:
   ```powershell
   # NSSM will be automatically downloaded and installed by the service installers
   # Default location: C:\nssm\win64\nssm.exe
   ```
...
5. **Service Management**:
   ...
   # Stop services
   net stop FlaskBackend
   net stop NextJSFrontend

   # Check status
   sc query FlaskBackend
   sc query NextJSFrontend
   ```
...
1. **Backend Environment (.env)**:
   ...
   NODE_ENV=production
   SSL_CERT_PATH=C:\Certbot\live\your-domain\fullchain.pem
   SSL_KEY_PATH=C:\Certbot\live\your-domain\privkey.pem
   ```
...
2. **Frontend Environment (.env.local)**:
   ...
   SSL_CRT_FILE=C:\Certbot\live\your-domain\fullchain.pem
   SSL_KEY_FILE=C:\Certbot\live\your-domain\privkey.pem
   ```

### Production Monitoring

1. **Service Logs**:
   - Backend: `C:\FlaskApps\forex_news_notifier\logs\app.log`
   - Frontend: `C:\FlaskApps\forex_news_notifier\frontend\logs\service-output.log`
   - Frontend Errors: `C:\FlaskApps\forex_news_notifier\frontend\logs\service-error.log`

2. **Windows Event Viewer**:
   - Check `Windows Logs > Application` for service events
   - Filter by source: "FlaskBackend" or "NextJSFrontend"

3. **Performance Monitoring**:
   - Use Task Manager > Services tab
...
1. **Service Won't Start**:
   ...
   nssm dump FlaskBackend
   nssm dump NextJSFrontend

   # View application event log
   Get-EventLog -LogName Application -Source "FlaskBackend" -Newest 10
   ```

2. **SSL Certificate Issues**:
   ```powershell
   # Test certificate access
   openssl x509 -in C:\Certbot\live\your-domain\fullchain.pem -text -noout
...
3. **Permission Issues**:
   ...
   icacls "C:\Certbot\live\your-domain" /grant "NT AUTHORITY\SYSTEM":(R)
   icacls "C:\FlaskApps\forex_news_notifier" /grant "NT AUTHORITY\SYSTEM":(F) /T
   ```
